...
export async function up({ context }: { context: MigrationContext }): Promise<void> {
  ...
  await db.query(
    'ALTER TABLE sales MODIFY COLUMN appointment_id INT NULL, MODIFY COLUMN customer_id INT NOT NULL',
  );
  await db.query('ALTER TABLE loyalty_ledger MODIFY COLUMN customer_id INT NOT NULL');
  await db.query(
    'ALTER TABLE notifications_log MODIFY COLUMN customer_id INT NULL, MODIFY COLUMN appointment_id INT NULL',
  );

  // 4. Restaurar llaves foraneas
  await db.query(
    'ALTER TABLE appointments ADD CONSTRAINT fk_appointments_customer FOREIGN KEY (customer_id) REFERENCES customers(id) ON DELETE RESTRICT ON UPDATE CASCADE',
  );
  await db.query(
    'ALTER TABLE appointment_services ADD CONSTRAINT fk_apt_services_apt FOREIGN KEY (appointment_id) REFERENCES appointments(id) ON DELETE CASCADE ON UPDATE CASCADE',
  );
  await db.query(
    'ALTER TABLE sales ADD CONSTRAINT fk_sales_appointment FOREIGN KEY (appointment_id) REFERENCES appointments(id) ON DELETE SET NULL ON UPDATE CASCADE',
  );
  await db.query(
    'ALTER TABLE sales ADD CONSTRAINT fk_sales_customer FOREIGN KEY (customer_id) REFERENCES customers(id) ON DELETE RESTRICT ON UPDATE CASCADE',
  );
  await db.query(
    'ALTER TABLE loyalty_ledger ADD CONSTRAINT fk_loyalty_ledger_customer FOREIGN KEY (customer_id) REFERENCES customers(id) ON DELETE RESTRICT ON UPDATE CASCADE',
  );
  await db.query(
    'ALTER TABLE notifications_log ADD CONSTRAINT fk_notifications_customer FOREIGN KEY (customer_id) REFERENCES customers(id) ON DELETE SET NULL ON UPDATE CASCADE',
  );
  await db.query(
    'ALTER TABLE notifications_log ADD CONSTRAINT fk_notifications_appointment FOREIGN KEY (appointment_id) REFERENCES appointments(id) ON DELETE SET NULL ON UPDATE CASCADE',
  );
...
export async function down({ context }: { context: MigrationContext }): Promise<void> {
  ...
  await db.query(
    'ALTER TABLE appointment_services MODIFY COLUMN appointment_id VARCHAR(64) NOT NULL',
  );
  await db.query(
    'ALTER TABLE sales MODIFY COLUMN appointment_id VARCHAR(64) NULL, MODIFY COLUMN customer_id VARCHAR(64) NOT NULL',
  );
  await db.query('ALTER TABLE loyalty_ledger MODIFY COLUMN customer_id VARCHAR(64) NOT NULL');
  await db.query(
    'ALTER TABLE notifications_log MODIFY COLUMN customer_id VARCHAR(64) NULL, MODIFY COLUMN appointment_id VARCHAR(64) NULL',
  );
