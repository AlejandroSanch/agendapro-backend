...
export async function up({ context }: { context: MigrationContext }): Promise<void> {
  ...
  await db.query('ALTER TABLE staff ADD CONSTRAINT fk_staff_role FOREIGN KEY (role_id) REFERENCES roles(id) ON DELETE RESTRICT ON UPDATE CASCADE');
  await db.query('ALTER TABLE staff_services ADD CONSTRAINT fk_staff_services_staff FOREIGN KEY (staff_id) REFERENCES staff(id) ON DELETE CASCADE ON UPDATE CASCADE');
  await db.query('ALTER TABLE staff_schedules ADD CONSTRAINT fk_staff_schedules_staff FOREIGN KEY (staff_id) REFERENCES staff(id) ON DELETE CASCADE ON UPDATE CASCADE');
  await db.query('ALTER TABLE staff_time_off ADD CONSTRAINT fk_staff_time_off_staff FOREIGN KEY (staff_id) REFERENCES staff(id) ON DELETE CASCADE ON UPDATE CASCADE');
  await db.query('ALTER TABLE appointment_services ADD CONSTRAINT fk_apt_services_staff FOREIGN KEY (staff_id) REFERENCES staff(id) ON DELETE RESTRICT ON UPDATE CASCADE');
  await db.query('ALTER TABLE commission_rules ADD CONSTRAINT fk_comm_rules_staff FOREIGN KEY (staff_id) REFERENCES staff(id) ON DELETE CASCADE ON UPDATE CASCADE');
  await db.query('ALTER TABLE commissions ADD CONSTRAINT fk_commissions_staff FOREIGN KEY (staff_id) REFERENCES staff(id) ON DELETE RESTRICT ON UPDATE CASCADE');
  await db.query('ALTER TABLE inventory_logs ADD CONSTRAINT fk_inventory_logs_staff FOREIGN KEY (staff_id) REFERENCES staff(id) ON DELETE SET NULL ON UPDATE CASCADE');
...
export async function down({ context }: { context: MigrationContext }): Promise<void> {
  ...
  await db.query(`INSERT IGNORE INTO roles (id, name) VALUES ('role_admin', 'admin'), ('role_staff', 'staff'), ('role_viewer', 'viewer')`);
