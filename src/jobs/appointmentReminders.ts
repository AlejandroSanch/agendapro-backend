...
export async function runRemindersJob() {
  ...
  try {
    ...
    const [users] = await controlDb.query<RowDataPacket[]>(
      'SELECT id, tenant_db_name, name FROM users WHERE tenant_db_name IS NOT NULL AND tenant_db_name != ""',
    );
    ...
    for (const user of users) {
      ...
      try {
        ...
        const [userRows] = await controlDb.query<RowDataPacket[]>(
          'SELECT business_name, name FROM users WHERE tenant_db_name = ? LIMIT 1',
          [tenantDbName],
        );
        ...
        for (const apt of appointmentsToRemind) {
          ...
          const startTimeStr = startDate.toLocaleString('es-MX', {
            dateStyle: 'short',
            timeStyle: 'short',
          });
          ...
          const timeFormatted = startDate.toLocaleTimeString('es-MX', {
            hour: '2-digit',
            minute: '2-digit',
          });
          ...
          if (apt.customer_email && process.env.SMTP_USER && process.env.SMTP_PASS) {
            const hasEmailLog = await notificationExists(
              controlDb,
              tenantDbName,
              apt.appointment_id,
              'email',
            ...
            if (!hasEmailLog) {
              try {
                ...
                logger.error(
                  { err: e, tenant: tenantDbName, appointmentId: apt.appointment_id },
          ...
          if (apt.customer_phone) {
            const hasWALog = await notificationExists(
              controlDb,
              tenantDbName,
              apt.appointment_id,
              'whatsapp',
            ...
            if (!hasWALog) {
              try {
                ...
                await logNotification(
                  ...
                  apt.appointment_id,
                  'whatsapp',
                  'Recordatorio WA Meta',
                  'Template: appointment_reminder',
                  'sent',
                );
                logger.info(
                  { tenant: tenantDbName, customer: customerName },
                  'WhatsApp reminder sent',
                );
              } catch (e: unknown) {
                const errMsg = e instanceof Error ? e.message : String(e);
                logger.error(
                  { err: e, tenant: tenantDbName, customer: customerName },
        ...
        logger.error(
          { err: tenantErr, tenant: tenantDbName },
          'Error processing tenant in reminders job',
        );
      }
    }
