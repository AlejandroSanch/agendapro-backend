...
export const PublicService = {
    ...
    for (const t of tenants) {
      ...
      if (rows.length > 0) {
        ...
        try {
          ...
          logger.warn(
            { err, appointmentId, tenantDbName: t.tenant_db_name },
            'Failed to backfill appointment_tenant_map',
          );
        }
    ...
    try {
      const [aptInfo] = await db.query<RowDataPacket[]>(
        `SELECT a.service_name, c.first_name, c.last_name FROM ${q(foundTenant)}.appointments a 
         JOIN ${q(foundTenant)}.customers c ON a.customer_id = c.id WHERE a.id = ?`,
        [appointmentId],
      );
      const customerName = aptInfo[0]
        ? `${aptInfo[0].first_name} ${aptInfo[0].last_name}`
        : 'Cliente';
  ...
  async getAppointmentDetails(appointmentId: string): Promise<PublicAppointmentDetails> {
    ...
    const [userRows] = await db.query<RowDataPacket[]>(
      'SELECT business_name, name FROM users WHERE tenant_db_name = ?',
      [foundTenant],
    );
    ...
    return {
      ...
      serviceName: cleanDeletedName(
        apt.service_name || apt.service_name_ref || 'Servicio Profesional',
      ),
      specialistName:
        `${apt.specialist_name || ''} ${apt.specialist_last_name || ''}`.trim() ||
