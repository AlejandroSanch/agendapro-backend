...
describe('GoogleCalendarService', () => {
  ...
  const mockOAuth2Client = {
    ...
    getToken: jest.fn().mockResolvedValue({
      tokens: { access_token: 'at', refresh_token: 'rt', expiry_date: 123456789 },
    }),
  ...
  describe('getCalendarClient', () => {
    ...
    it('debería configurar el cliente con tokens de la DB', async () => {
      mockDb.query.mockResolvedValueOnce([
        [{ access_token: 'at', refresh_token: 'rt', expires_at: new Date() }],
      ]);
      const client = await GoogleCalendarService.getCalendarClient(userId);
  ...
  describe('pushEvent', () => {
    ...
    it('debería insertar un evento en Google Calendar', async () => {
      jest
        .spyOn(GoogleCalendarService, 'getCalendarClient')
        .mockResolvedValue(mockCalendarClient as any);
