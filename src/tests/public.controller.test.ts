...
describe('PublicController Integration', () => {
  ...
  it('debería confirmar cita usando el fast-path (lookup table)', async () => {
    ...
    expect(createSystemNotification).toHaveBeenCalledWith('tenant_fast', expect.objectContaining({
  ...
  it('debería confirmar cita usando el slow-path (scan) y backfill el map', async () => {
    ...
    mockQuery.mockResolvedValueOnce([[{ tenant_db_name: 'tenant_a' }, { tenant_db_name: 'tenant_b' }]]);
    ...
    // 7. Notification info
    mockQuery.mockResolvedValueOnce([[{ first_name: 'Maria', last_name: 'Gomez', service_name: 'Tinte' }]]);
    ...
    const backfillCall = mockQuery.mock.calls.find(call => call[0].includes('INSERT IGNORE INTO appointment_tenant_map'));
