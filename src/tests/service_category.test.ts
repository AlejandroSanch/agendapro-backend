import { listServices, createService, updateService, deleteService } from '../data/repositories/service.repository';
import { listCategories, createCategory, updateCategory, deleteCategory } from '../data/repositories/category.repository';
...
describe('Service & Category Repositories', () => {
  ...
  describe('ServiceRepository', () => {
    it('debería listar servicios correctamente mapeando a ServiceRecord', async () => {
      ...
      expect(mockQuery).toHaveBeenCalledWith(expect.stringMatching(/FROM\s+`tenant_test`\.services/i));
    });
    ...
    it('debería actualizar un servicio y resolver categoría existente', async () => {
      ...
      mockQuery.mockResolvedValueOnce([[
        { id: '100', name: 'Tinte', category: 'Color', category_id: '20', duration_minutes: 60, price_cents: 5000, display_order: 6, is_active: 1 }
      ]]);
      ...
      mockQuery.mockResolvedValueOnce([[
        { id: '100', name: 'Tinte Plus', category: 'Premium', category_id: '30', duration_minutes: 90, price_cents: 7000, display_order: 6, is_active: 1 }
      ]]);
      ...
      expect(mockQuery).toHaveBeenCalledWith(
        expect.stringMatching(/UPDATE\s+`tenant_test`\.services\s+SET\s+name\s+=\s+\?,\s+category_id\s+=\s+\?/),
  ...
  describe('CategoryRepository', () => {
    ...
    it('debería crear una categoría correctamente', async () => {
      ...
      mockQuery.mockResolvedValueOnce([[{ id: '50', name: 'Spa', description: 'Relax', type: 'service' }]]);
    ...
    it('debería actualizar una categoría correctamente', async () => {
      mockQuery.mockResolvedValueOnce([{ affectedRows: 1 }]);
      mockQuery.mockResolvedValueOnce([[{ id: '50', name: 'Spa Deluxe', description: 'Ultra relax', type: 'service' }]]);
    ...
    it('debería lanzar error al intentar eliminar categoría con servicios asociados', async () => {
      ...
      await expect(deleteCategory(userId, '10')).rejects.toThrow('No se puede eliminar la categoría porque tiene servicios asociados.');
    });
    ...
    it('debería lanzar error al intentar eliminar categoría con productos asociados', async () => {
      ...
      await expect(deleteCategory(userId, '10')).rejects.toThrow('No se puede eliminar la categoría porque tiene productos asociados.');
    });
